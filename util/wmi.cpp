...
namespace util::wmi {
	...
	bool load() {
		...
		if (FAILED(result)) {
			LOG_ERROR(XOR("Failed to initialize service (#1)").c_str());
			return false;
		}
		...
		if (FAILED(result)) {
			LOG_ERROR(XOR("Failed to initialize service (#2) %X").c_str(), result);
			CoUninitialize();
			return false;
		}
		...
		if (FAILED(result)) {
			LOG_ERROR(XOR("Failed to initialize service (#3) %X").c_str(), result);
			g_locator->Release();
			CoUninitialize();
			return false;
		}
		...
		if (FAILED(result)) {
			LOG_ERROR(XOR("Failed to initialize service (#4) %X").c_str(), result);
			g_services->Release();
			g_locator->Release();
			CoUninitialize();
			return false;
