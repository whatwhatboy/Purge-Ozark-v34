...
namespace util::log {
	...
	void load() {
		AllocConsole();
		SetConsoleTitleA(XOR("Purge  Menu | Version -  Alpha 0.0.1").c_str());

		FILE* stream;
		freopen_s(&stream, XOR("CONIN$").c_str(), XOR("w").c_str(), stdin);
		freopen_s(&stream, XOR("CONOUT$").c_str(), XOR("w").c_str(), stdout);
		freopen_s(&stream, XOR("CONOUT$").c_str(), XOR("w").c_str(), stderr);
