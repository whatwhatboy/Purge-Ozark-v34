...
void button_option::render(int position) {
	...
	//	menu::renderer::draw_sprite({ "Purgetextures", "keyboard.png" }, { global::ui::g_position.x + 0.209f - (0.22f - global::ui::g_scale.x), global::ui::g_position.y + 0.016f + (position * global::ui::g_option_scale) }, { 0.015f * 0.9f, 0.022f * 0.9f }, 0.f, { 255, 255, 255, 255 });
...
void button_option::render_selected(int position, std::stack<std::string> submenu_name_stack) {
	...
	if (m_requirement() && menu::input::is_option_pressed()) {
		...
		if (m_keyboard.m_enabled) {
			...
			native::display_onscreen_keyboard(0, "Purge", "", m_keyboard.m_default_text.c_str(), "", "", "", m_keyboard.m_max_chars);
		}
	}
	...
	if (m_keyboard.m_enabled) {
		if (!m_keyboard.m_is_active) {
			...
			util::fiber::pool::add([this] {
				native::display_onscreen_keyboard(0, "Purge", "", "", "", "", "", m_keyboard.m_max_chars);
