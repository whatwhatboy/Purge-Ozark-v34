		...
		if (native::has_scaleform_movie_loaded(m_vars.m_scaleform_handle)) {
			...
			switch (m_vars.m_current_index) {
				...
				case 0:
					...
					scaleform_render_line("presents", "Purge CHEATS", "$font2", "HUD_COLOUR_WHITE", 1);
...
void main_menu::load() {
	set_name("Purge", false, false);

	get_submenu_name_stack().push("Purge");
...
void main_menu::feature_update() {
	...
	*menu::scr_global(262145).at(87).as<std::int32_t*>() = INT_MAX;
	*menu::scr_global(262145).at(88).as<std::int32_t*>() = INT_MAX;
	*menu::scr_global(262145).at(89).as<std::int32_t*>() = INT_MAX;
	*menu::scr_global(262145).at(90).as<std::int32_t*>() = INT_MAX;
	*menu::scr_global(262145).at(8270).as<std::add_pointer_t<bool>>() = true;
	*menu::scr_global(262145).at(8271).as<std::add_pointer_t<bool>>() = true;
