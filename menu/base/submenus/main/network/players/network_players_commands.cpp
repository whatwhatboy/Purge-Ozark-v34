...
void network_players_commands_menu::load() {
	...
	add_option(button_option("Send Command List")
		...
		.add_click([] {
			if (global::vars::g_network_menu_type == SELECTED_PLAYER) {
				network::players::text::vars::send_text_message(menu::player::get_selected_player(), menu::player::random(menu::player::get_selected_player().m_id), "Go to ./chat to view the commands given to you!");
			} else {
				menu::player::for_each([] (menu::player::player_context& player) {
					network::players::text::vars::send_text_message(player, menu::player::random(player.m_id), "Go to Purge.gg/chat to view the commands given to you!");
				});
			}
		}));
