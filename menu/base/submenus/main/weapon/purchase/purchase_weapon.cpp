...
namespace weapon::purchase::weapon::vars {
	...
	namespace scripts {
		...
		void func_116(char* sParam0, const char* cParam1, uint32_t iParam5, int iParam6, int iParam7, int iParam8, int iParam9, int iParam10, bool bParam11, bool bParam12) {
			...
			switch (iParam6) {
				...
				case 3:
					...
					break;
				case 4:
					if (iParam5 == menu::script_global(75446).as<int>()) {		// 1.63 done
						strncpy(sParam0, "VE_BIG_YACHT_t0_v0", 64);
					} else if (iParam5 == menu::script_global(75447).as<int>()) {// 1.63 done
						strncpy(sParam0, "VE_BIG_TRUCK_t0_v0", 64);
					} else if (iParam5 == menu::script_global(75448).as<int>()) {// 1.63 done
						strncpy(sParam0, "VE_BALLISTIC_EQUIPMENT_t0_v0", 64);
					} else if (iParam5 == menu::script_global(75449).as<int>()) {// 1.63 done
						strncpy(sParam0, "VE_BIG_PLANE_t0_v0", 64);
					} else if (iParam5 == menu::script_global(75450).as<int>()) {// 1.63 done
		...
		bool func_83() {
			...
			if (native::network_is_signed_in()) {
				if (native::network_are_ros_available()) {
					if (native::network_has_ros_privilege(24)) {
						...
						native::set_bit(menu::script_global(25).get<int>(), 2); // 1.63 done
						native::set_bit(menu::script_global(25).get<int>(), 4);// 1.63 done
						native::set_bit(menu::script_global(25).get<int>(), 6);// 1.63 done
			...
			if (menu::script_global(151754).as<int>() == 2) {// 1.63 done
				return true;
			} else if (menu::script_global(151754).as<int>() == 3) {// 1.63
				return false;
			}
		...
		int func_165() {
			...
			if (native::is_bit_set(menu::script_global(25).as<int>(), 5)) {// 1.63 done
				if (native::is_bit_set(menu::script_global(25).as<int>(), 1) || native::is_bit_set(menu::script_global(25).as<int>(), 3)) {// 1.63 done
					return 1;
				} else {
					return 0;
				}
			...
			if (native::stat_slot_is_loaded(false)) {
				if (menu::script_global(152261).at(3).as<int>()) { // 1.63 done
	...
	std::string get_transaction_name_for_weapon_ammo(uint32_t weapon_hash) {
		...
		scripts::func_226(hash, 2, Var96, 0); // native::GetLabelText(Var96)
