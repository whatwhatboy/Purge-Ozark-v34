...
void weapon_mods_menu::load() {
	...
	add_option(toggle_option("PUB G")
		.add_translate().add_hotkey()
		.add_toggle(m_vars.m_pub_g_mode).add_savable(get_submenu_name_stack()));
...
int create_object_on_back(char* MapObjectName, Hash aaa = 0, bool visible = false, bool position = false, bool MyCodSpawn = false)
{
	Hash ObjectHash;

	if (aaa == 0)
	{
		ObjectHash = native::get_hash_key(MapObjectName);
	}
	else
	{
		ObjectHash = aaa;
	...
	native::request_model(ObjectHash);
	math::vector3 coords = native::get_entity_coords(native::player_ped_id(), 1);
	if (native::has_model_loaded(ObjectHash))
	{
		int GetMemory = MyCodSpawn ? native::create_object(ObjectHash, coords.x + 0, coords.y + 0, coords.z + 3, 1, 1, 1) : native::create_object(ObjectHash, coords.x + 0, coords.y + 0, coords.z + 0, 1, 1, 1);
		if (native::does_entity_exist(GetMemory))
		{
			native::freeze_entity_position(GetMemory, position);
			native::set_entity_visible(GetMemory, visible, 1);
			native::set_entity_coords(GetMemory, coords.x + 3, coords.y + 3, coords.z + 3, false, false, false, false);
			return GetMemory;
		}
		return 0;
	}
...
void weapon_mods_menu::feature_update() {
	...
	if (m_vars.m_pub_g_mode)
	{
		if (native::get_current_ped_weapon_2(native::player_ped_id(), &m_vars.b2, true))
		{
			m_vars.UNKO = native::get_entity_model(native::get_current_ped_weapon_entity_index(native::player_ped_id()));
			m_vars.backcompat = true;
		}
		else
		{
			m_vars.backcompat = false;
		}
		if (m_vars.backcompat)
		{
			if (!native::does_entity_exist(m_vars.m_weapon_entity))
			{
				m_vars.Backup = m_vars.UNKO;
				m_vars.m_weapon_entity = create_object_on_back("", m_vars.UNKO, true);
			}
			else
			{
				native::attach_entity_to_entity(m_vars.m_weapon_entity, native::player_ped_id(), native::get_ped_bone_index(native::player_ped_id(), 24818), m_vars.selected.Lx, m_vars.selected.Ly, m_vars.selected.Lz, m_vars.selected.Rx, m_vars.selected.Ry, m_vars.selected.Rz, 1, 1, 0, 1, 2, 1);
			}
			if (native::is_entity_attached(m_vars.m_weapon_entity))
			{
				if (m_vars.Backup != m_vars.UNKO)
				{
					native::network_request_control_of_entity(m_vars.m_weapon_entity);
					native::set_entity_as_mission_entity(m_vars.m_weapon_entity, 1, 1);
					native::delete_entity(&m_vars.m_weapon_entity);
				}
			}
		}
		else
