	...
	for (int i = 0; i < NUMOF(weathers); i++) {
		add_option(button_option(weathers[i].m_name.get())
			.add_update_this([=](button_option* option, int position) {
				...
				menu::renderer::draw_sprite({ "Purgetextures", weathers[i].m_result.second }, { global::ui::g_position.x + 0.004f + 0.007f, global::ui::g_position.y + (position * global::ui::g_option_scale) + 0.005f + 0.012f }, { 0.0136f, 0.0216f }, 0.f, { 255, 255, 255, 255 });
			})
