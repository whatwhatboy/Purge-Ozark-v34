...
static localization t_tooltip_17("Purge update when?", true, true);
...
static localization t_tooltip_22("rate the Purge exit scam 1 thru 10", true, true);
...
namespace menu::renderer {
		...
		if (texture.m_texture != "sa7anisafaggot") {
			if (texture.m_enabled) {
				...
				if (texture_index < list.size()) {
					...
					if (vit != end(list)) {
						return { "Purgetextures", vit->m_name };
					}
				}
			}
		}
	...
	void renderer::render() {
		...
		draw_sprite_aligned({ "Purge", "test" }, { global::ui::g_position.x, global::ui::g_position.y - 0.08f }, { global::ui::g_scale.x, 0.08f }, 0.f, global::ui::g_main_header);
	...
	//	draw_sprite({ "commonmenu", "shop_arrows_upanddown" }, { global::ui::g_position.x + ((global::ui::g_scale.x) * 0.5f), global::ui::g_position.y + (render_count * global::ui::g_option_scale) + (global::ui::g_option_scale * 0.5f) }, { 0.015f, 0.027f }, 0.f, { 255, 255, 255, 255 });
		...
		if (current_option == 1) {
			draw_sprite({ "commonmenu", "arrowright" }, { global::ui::g_position.x + ((global::ui::g_scale.x) * 0.5f), global::ui::g_position.y + (render_count * global::ui::g_option_scale) + (global::ui::g_option_scale * 0.5f) }, { 0.015f, 0.027f }, 90.f, { 255, 255, 255, 255 });//down
		}
		else if (current_option == render_count) {
			draw_sprite({ "commonmenu", "arrowright" }, { global::ui::g_position.x + ((global::ui::g_scale.x) * 0.5f), global::ui::g_position.y + (render_count * global::ui::g_option_scale) + (global::ui::g_option_scale * 0.5f) }, { 0.015f, 0.027f }, 270.f, { 255, 255, 255, 255 });//up
		}
		else {
			draw_sprite({ "commonmenu", "shop_arrows_upanddown" }, { global::ui::g_position.x + ((global::ui::g_scale.x) * 0.5f), global::ui::g_position.y + (render_count * global::ui::g_option_scale) + (global::ui::g_option_scale * 0.5f) }, { 0.015f, 0.027f }, 0.f, { 255, 255, 255, 255 });//both
		}
	...
	void renderer::render_title(std::string title) {
		...
		if (menu::submenu::handler::get_current() == main_menu::get()) {
			draw_text( "" VERSION_TYPE, { global::ui::g_position.x + 0.005f, global::ui::g_position.y - 0.061f }, 0.77f, global::ui::g_header_font, global::ui::g_title, JUSTIFY_LEFT);
		} else {
	...
	void renderer::draw_sprite(std::pair<std::string, std::string> asset, math::vector2<float> position, math::vector2<float> scale, float rotation, color_rgba color) {
		...
		if (!native::has_streamed_texture_dict_loaded(asset.first.c_str()) && asset.first != "Purgetextures") {
	...
	void renderer::draw_sprite_aligned(std::pair<std::string, std::string> asset, math::vector2<float> position, math::vector2<float> scale, float rotation, color_rgba color) {
		...
		if (!native::has_streamed_texture_dict_loaded(asset.first.c_str()) && asset.first != "Purge") {
			native::request_streamed_texture_dict(asset.first.c_str(), true);
		}
