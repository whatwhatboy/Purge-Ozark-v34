...
int menu::hooks::apply_vehicle_data__crash(rage::network::net_object* object, uint64_t node_data) {
	if (*(bool*)(node_data + 0x119)) {
		uint64_t net_obj = *(uint64_t*)((uint64_t)object - 0x1B0/*1.64*/);
		...
		if (is_valid_vtable(vtable)) {
			if (caller::call<uint64_t>(*(uint64_t*)(vtable + 1592)/*1.64*/, net_obj) == 0) {
				LOG("Someone may have just tried to crash you");
				return 0;
			}
		}
...
void menu::hooks::crash_protect_model(int64_t a1, int64_t a2, int a3, char a4)
{
	if (!*(int64_t*)(a1 + 0xD8))
		return;
	
	
	return crash_protect_model_t(a1, a2, a3, a4);
}
