...
#include "menu/base/submenus/main/network/chat/network_global_chat_Purge.h"
...
bool menu::hooks::read_chat_message(rage::network::global_msg_context* context, rage::bit_buffer* bit_buffer) {
	...
	if (sender_id != -1) {
		...
		LOG("Chat", "[%s]%s %s", menu::player::get_player(sender_id).m_name.c_str(), spoof.c_str(), context->m_message);
...
bool menu::hooks::receive_text_message(rage::network::net_msg_text_message* identifier, rage::bit_buffer* bit_buffer) {
	...
	if (sender_id != -1) {
		...
		LOG("Text", "[%s]%s %s", menu::player::get_player(sender_id).m_name.c_str(), spoof.c_str(), identifier->m_message);
	}
