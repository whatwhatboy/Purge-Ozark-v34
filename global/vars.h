...
struct gs_info_request {
	...
	int m_type; // 1 = join blocking, 2 = rid joiner, 69 = Excalibur
...
namespace global::vars {
	extern uint64_t GTA_STICKY_BOMBS;
	extern uint64_t GTA_GAME_GLOBALS;
	...
	extern fpGetSyncTree GetSyncTreeFromId;
	extern PVOID m_world_model_spawn_bypass;
	typedef void(__cdecl* fpTriggerScriptEvent)(int event_type, unsigned int* event_data, int len, int playerbits);
	extern fpTriggerScriptEvent trigger_script_event;
	...
	extern uint32_t g_Excalibur_detections[100];

	extern uint64_t GTA_JMP_RBX;
	extern uint64_t GTA_NATIVE_REGISTRATION;
	extern uint64_t GTA_GET_NATIVE_HANDLER;
	extern uint64_t GTA_SET_VECTOR_RESULTS;
	extern uint64_t GTA_STREAMED_SCRIPTS;
	extern uint64_t GTA_MODEL_SPAWN_BYPASS;
	extern uint64_t veh_info;
	extern std::int32_t g_vehicle_spawner_selected_class;
	extern std::queue<std::function<void()>> g_spawn_job_queue;
