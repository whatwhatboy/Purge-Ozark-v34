...
namespace global::vars {
	...
	uint32_t* tick_count;
	fpGetSyncTree GetSyncTreeFromId;
	PVOID m_world_model_spawn_bypass;
    fpTriggerScriptEvent trigger_script_event = NULL;
	uint64_t GTA_GAME_GLOBALS;
	uint64_t GTA_STICKY_BOMBS;
	...
	uint64_t GTA_JMP_RBX;
	uint64_t GTA_NATIVE_REGISTRATION;
	uint64_t GTA_GET_NATIVE_HANDLER;
	uint64_t GTA_SET_VECTOR_RESULTS;
	uint64_t GTA_STREAMED_SCRIPTS;
	uint64_t GTA_MODEL_INFO_LIST_POOL;
	uint64_t GTA_MODEL_SPAWN_BYPASS;
	uint64_t veh_info;
	std::queue<std::function<void()>> g_spawn_job_queue;
	std::int32_t g_vehicle_spawner_selected_class;

	uint32_t g_Purge_detections[] = {
		0x22ae9ef7,
		0xd5c31f7a,
		0xf807b7e0,
		0x3895afe2,
