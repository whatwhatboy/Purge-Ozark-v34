...
const char* api::make_token( const char* key ) {
   ...
   hwidstuff = serial();
    ...
    sprintf_s( token_buf, "%s%s%s", key, hwidstuff.c_str(), time_buffer );
...
bool api::auth( const char* key) {
    hwidstuff = serial();
    httplib::Client client( "77.68.117.95" );
    char buf[ 1024 ];
    sprintf_s( buf, "method=auth&key=%s&hwid=%s", key, hwidstuff.c_str());
    if ( auto res = client.Post( "/site/api/api.php", buf, "application/x-www-form-urlencoded" ) )
    {
...
std::string api::get_loader( ) {
    httplib::Client client( "77.68.117.95" ); // Your hostname 

    char buf[ 1024 ];

    printf_s( "%s Response buffer: %s\n", __FUNCTION__, buf );

    if ( auto res = client.Post( "j/site/api/api.php", "method=getloader", "application/x-www-form-urlencoded" ) ) {
...
bool api::check_version( )
{
    ...
    httplib::Result res = client.Post( "/site/api/api.php", buf, "application/x-www-form-urlencoded" );
